...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl gs1 cin sh"
		...
		<xsl:choose>
			<xsl:when test="$debug = 1">
				<Output>
					...
					<xsl:choose>
						<xsl:when test="cin:catalogueItemNotificationMessage">
							<xsl:apply-templates select="cin:catalogueItemNotificationMessage"/>
						</xsl:when>
						<xsl:otherwise>
							<Error Count="{count(*/*)}">
								<xsl:value-of select="concat('Provided xml is not valid for this ruleset ', local-name(*), ' xmlns=', namespace-uri(*))"/>
							</Error>
						</xsl:otherwise>
					</xsl:choose>
	...
	<xsl:template match="cin:catalogueItemNotificationMessage">
		<xsl:apply-templates select="sh:StandardBusinessDocumentHeader"/>
	...
	<xsl:template match="sh:StandardBusinessDocumentHeader">
		<xsl:apply-templates select="gs1:SetResponseId(sh:DocumentIdentification/sh:InstanceIdentifier)"/>
	</xsl:template>
	...
	<xsl:template match="*" mode="r449">
		...
		<xsl:if test="preliminaryItemStatusCode != 'PRELIMINARY' and $command = 'CHANGE_BY_REFRESH'">
			<xsl:variable name="xpath" select="tradeItemInformation/extension/*[namespace-uri()='urn:gs1:gdsn:trade_item_measurements:xsd:3' and local-name()='tradeItemMeasurementsModule']/tradeItemMeasurements/netContent"/>
			<xsl:if test="$xpath">
				<xsl:apply-templates select="$xpath" mode="error">
	...
	<xsl:template match="*" mode="r450">
		...
		<xsl:if test="preliminaryItemStatusCode != 'PRELIMINARY' and $command = 'CHANGE_BY_REFRESH'">
			<xsl:variable name="xpath" select="tradeItemInformation/extension/*[namespace-uri()='urn:gs1:gdsn:sales_information:xsd:3' and local-name()='salesInformationModule']/salesInformation/priceComparisonMeasurement"/>
			<xsl:if test="$xpath">
				<xsl:apply-templates select="$xpath" mode="error">
	...
	<xsl:template match="*" mode="r451">
		...
		<xsl:if test="preliminaryItemStatusCode != 'PRELIMINARY' and $command = 'CHANGE_BY_REFRESH'">
			<xsl:variable name="xpath" select="tradeItemInformation/extension/*[namespace-uri()='urn:gs1:gdsn:trade_item_description:xsd:3' and local-name()='tradeItemDescriptionModule']/tradeItemDescriptionInformation/brandNameInformation/brandName"/>
			<xsl:if test="$xpath">
				<xsl:apply-templates select="$xpath" mode="error">
	...
	<xsl:template match="*" mode="r452">
		...
		<xsl:if test="preliminaryItemStatusCode != 'PRELIMINARY' and $command = 'CHANGE_BY_REFRESH'">
			<xsl:variable name="xpath" select="nextLowerLevelTradeItemInformation/totalQuantityOfNextLowerLevelTradeItem"/>
			<xsl:if test="$xpath">
				<xsl:apply-templates select="$xpath" mode="error">
	...
	<xsl:template match="*" mode="r453">
		...
		<xsl:if test="$command = 'CHANGE_BY_REFRESH'">
			<xsl:variable name="xpath" select="nextLowerLevelTradeItemInformation/childTradeItem/gtin"/>
			<xsl:if test="$xpath">
				<xsl:apply-templates select="$xpath" mode="error">
	...
	<xsl:template match="*" mode="r485">
		<xsl:param name="command"/>
		<!--Rule 485: discontinuedDateTime must only be updated  with documentCommand of 'CORRECT'.-->
		<xsl:if test="$command != 'CORRECT' and $command != 'ADD'">
			<xsl:variable name="xpath" select="tradeItemSynchronisationDates/discontinuedDateTime"/>
			<xsl:if test="$xpath != ''">
				<xsl:apply-templates select="$xpath" mode="error">
	...
	<xsl:template match="*" mode="r550">
		<!--Rule 550: If specialItemCode does not equal 'DYNAMIC_ASSORTMENT' and grossWeight is used, then grossWeight SHALL be greater than 0.96 multiplied by the ((Parent trade Item packagingWeight if used) + (Sum of (Parent trade item quantityOfNextLowerLevelTradeItem multiplied by Child GTIN grossWeight))).-->
		<xsl:if test="tradeItemInformation/extension/*[namespace-uri()='urn:gs1:gdsn:marketing_information:xsd:3' and local-name()='marketingInformationModule']/marketingInformation/specialItemCode != 'DYNAMIC_ASSORTMENT'">
		...
		<!--<xsl:variable name="xpath" select="/catalogue_item_notification:catalogueItemNotificationMessage/transaction/documentCommand/catalogue_item_notification:catalogueItemNotification/catalogueItem/tradeItem/nextLowerLevelTradeItemInformation/childTradeItem/quantityOfNextLowerLevelTradeItem" />

		<xsl:variable name="xpath2" select="/catalogue_item_notification:catalogueItemNotificationMessage/transaction/documentCommand/catalogue_item_notification:catalogueItemNotification/catalogueItem/tradeItem/tradeItemInformation/extension/*[namespace-uri()='urn:gs1:gdsn:packaging_information:xsd:3' and local-name()='packagingInformationModule']/packaging/packagingWeight" />

		<xsl:if test="$xpath = 'TODO'">
			<xsl:apply-templates select="." mode="error">
				<xsl:with-param name="id" select="550" />
			</xsl:apply-templates>
		</xsl:if>-->
	</xsl:template>
	...
	<xsl:template match="tradeItem" mode="instance_rules">
		...
		<xsl:apply-templates select="." mode="r449">
			<xsl:with-param name="command" select="$command"/>
		</xsl:apply-templates>
		<xsl:apply-templates select="." mode="r450">
			<xsl:with-param name="command" select="$command"/>
		</xsl:apply-templates>
		<xsl:apply-templates select="." mode="r451">
			<xsl:with-param name="command" select="$command"/>
		</xsl:apply-templates>
		<xsl:apply-templates select="." mode="r452">
			<xsl:with-param name="command" select="$command"/>
		</xsl:apply-templates>
		<xsl:apply-templates select="." mode="r453">
			<xsl:with-param name="command" select="$command"/>
		</xsl:apply-templates>
		...
		<xsl:apply-templates select="." mode="r485">
			<xsl:with-param name="command" select="$command"/>
		</xsl:apply-templates>
